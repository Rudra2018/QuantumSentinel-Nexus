...
def lambda_handler(event, context):
    """Enhanced unified Lambda handler with chunked upload support"""
        ...
        if 'httpMethod' in event:
            ...
            if method == 'POST' and '/upload' in path:
                ...
                if action == 'upload':
                    ...
                    return handle_chunk_upload(body)
                elif action == 'finalize_chunks':
                    return handle_finalize_chunks(body)
...
def handle_chunk_upload(body):
    ...
    try:
        ...
        if not all([upload_id, chunk_index is not None, chunk_data]):
            return create_response(400, {'error': 'Missing chunk data'})

        # Decode chunk
        chunk_content = base64.b64decode(chunk_data)

        # Store chunk in S3
        chunk_bucket = 'quantumsentinel-advanced-file-uploads'
        ...
        return create_response(500, {'error': f'Chunk upload failed: {str(e)}'})

def handle_finalize_chunks(body):
    """Finalize chunked upload and start analysis"""
    try:
        upload_id = body.get('upload_id')
        filename = body.get('filename')
        ...
        if not all([upload_id, filename, total_chunks]):
            return create_response(400, {'error': 'Missing finalization data'})

        # Reassemble file from chunks
        chunk_bucket = 'quantumsentinel-advanced-file-uploads'

        # List all chunks
        chunk_prefix = f"chunks/{upload_id}/"
        chunks_response = s3.list_objects_v2(
            Bucket=chunk_bucket,
            Prefix=chunk_prefix
        )

        if 'Contents' not in chunks_response or len(chunks_response['Contents']) != total_chunks:
            return create_response(400, {'error': 'Missing chunks for reassembly'})

        # Sort chunks by index
        chunks = sorted(chunks_response['Contents'], key=lambda x: x['Key'])
        ...
        for chunk_obj in chunks:
            chunk_response = s3.get_object(Bucket=chunk_bucket, Key=chunk_obj['Key'])
            chunk_data = chunk_response['Body'].read()
            file_parts.append(chunk_data)
            total_size += len(chunk_data)

        # Combine all chunks
        complete_file = b''.join(file_parts)
        ...
        # Clean up chunks
        for chunk_obj in chunks:
            s3.delete_object(Bucket=chunk_bucket, Key=chunk_obj['Key'])

        # Start analysis
        analysis_id = f"UNIFIED-ADV-{int(time.time())}"
        analysis_results = run_unified_analysis(complete_file, filename, analysis_id)

        # Store analysis metadata and results
        store_analysis_results(analysis_id, filename, file_key, len(complete_file), analysis_results)

        return create_response(200, {
            'analysis_id': analysis_id,
            'status': 'completed',
            'message': 'Chunked upload completed and analysis finished',
            'engines_executed': 14,
        ...
        return create_response(500, {'error': f'Finalization failed: {str(e)}'})
...
def handle_file_upload(body):
    ...
    try:
        ...
        if len(file_content) > 50 * 1024 * 1024:  # 50MB
            return create_response(400, {'error': 'File too large for direct upload. Use chunked upload.'})
...
def handle_list_analyses():
    ...
    try:
        ...
        response = s3.list_objects_v2(
            Bucket=results_bucket,
            Prefix='metadata/',
            MaxKeys=20  # Limit to 20 recent analyses
        )
